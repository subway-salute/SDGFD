...
from module import CNN_Frontend, DomainDiscriminator
...
def main():
    ...
    wandb.init(project="PU_Thesis_Final", name=f"DANN_{args.source}", config=vars(args))
    ...
    domain_disc = DomainDiscriminator(256).to(device)

    optimizer = torch.optim.Adam(list(fe.parameters()) + list(cls.parameters()) + list(domain_disc.parameters()),
                                 lr=0.001)
    ...
    for epoch in range(1, 41):
        ...
        for i, l in train_loader:
            ...
            d_src = domain_disc(f_src, alpha)
            d_tgt = domain_disc(f_tgt, alpha)
            loss_adv = criterion_bce(d_src, torch.zeros(B, 1).to(device)) + criterion_bce(d_tgt,
                                                                                          torch.ones(B, 1).to(device))

            loss = loss_cls + 1.0 * loss_adv
            loss.backward();
            optimizer.step();
        ...
        if epoch == 40:
            ...
            np.save(f"plot_data/features_DANN_{args.source}.npy", np.array(all_features))
            np.save(f"plot_data/labels_DANN_{args.source}.npy", np.array(all_labels))
            np.save(f"plot_data/preds_DANN_{args.source}.npy", np.array(all_preds))
