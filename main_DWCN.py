...
class LP(nn.Module):
    ...
    def forward(self, x, s1, s2):
        mu = x.mean(dim=2, keepdim=True); var = x.var(dim=2, keepdim=True)
        x_normed = (x - mu.detach()) / (var + 0.1).sqrt().detach()
        gamma = self.fc1(s1).view(s1.size(0), -1, 1); beta = self.fc2(s2).view(s2.size(0), -1, 1)
...
class Fea_Extraction(nn.Module):
    def __init__(self, is_teacher=False):
        ...
        self.w2 = DWConv(num_channels=16); self.c2 = SConv_1D(32, 32, 3)
...
class DWCN(nn.Module):
    def __init__(self, num_classes=3):
        super().__init__()
        self.G_te = Fea_Extraction(is_teacher=True); self.C_te = nn.Linear(32, num_classes)
        self.G_st = Fea_Extraction(is_teacher=False); self.C_st = nn.Linear(32, num_classes)
...
def main():
    ...
    wandb.init(project="PU_test2", name=f"DWCN_{args.source}2{args.target}")
    ...
    for epoch in range(1, 41):
        ...
        for inputs, labels in train_loader:
            ...
            opt_te.zero_grad(); opt_st.zero_grad()

            f_te = model.G_te(inputs, perturb=True); f_st = model.G_st(inputs)
            ...
            (loss_c + loss_kl).backward(); opt_te.step(); opt_st.step()

            _ = model.G_te(inputs, perturb=True); _ = model.G_st(inputs)
            loss_ccp = 0.05 * (-F_distance(gram(model.G_st.l0), gram(model.G_te.l0)))
            opt_ld.zero_grad(); loss_ccp.backward(); opt_ld.step()
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                ...
                if len(inputs.shape) == 2: inputs = inputs.unsqueeze(1)
                logits = model.C_st(model.G_st(inputs))
    ...
    print(f"DWCN_FINAL_ACCURACY:{np.mean(tail_acc):.2f}")
