...
class ADAIN(nn.Module):
    def __init__(self):
        ...
        self.fc_layer1 = nn.Linear(1, 256); self.fc_layer2 = nn.Linear(1, 256)
    def forward(self, x, z, y):
        x = x.unsqueeze(1)
        gamma = self.fc_layer1(z).unsqueeze(1); beta = self.fc_layer2(y).unsqueeze(1)
...
class AdversarialNetwork(nn.Module):
    def __init__(self, in_feature, hidden_size=300):
        ...
        self.relu1 = nn.ReLU(); self.dropout1 = nn.Dropout(0.5); self.dropout2 = nn.Dropout(0.5)
        self.iter_num = 0; self.max_iter = 3300.0
    def forward(self, x):
        if self.training:
            self.iter_num += 1
            coeff = calc_coeff(self.iter_num, 1.0, 0.0, 10, self.max_iter)
            x = x * 1.0; x.register_hook(grl_hook(coeff))
...
class GFCD(nn.Module):
    ...
    def forward(self, x, z=None, y=None):
        ...
        if self.training:
            feat_new1 = self.adain(feat, z, y); feat_new2 = self.adain(feat.detach().clone(), z, y)
            x_class = torch.cat([feat, feat_new1], 0); x_domain = torch.cat([feat, feat_new2], 0)
            x_di_class = self.DI(x_class); x_di_domain = self.DI(x_domain)
...
def main():
    ...
    set_seed(42); device = torch.device('cuda')

    wandb.init(project="PU_test2", name=f"DACN_{args.source}2{args.target}")
    train_loader = construct_loader('./data', 'PU', args.source, 40, True)
    test_loader = construct_loader('./data', 'PU', args.target, 40, False)

    net = GFCD().to(device)
    criterion_cls = nn.CrossEntropyLoss(); criterion_bce = nn.BCEWithLogitsLoss()
    ...
    for epoch in range(1, 41):
        ...
        for inputs, labels in train_loader:
            B = inputs.size(0); inputs, labels = inputs.to(device).float(), labels.to(device)
            if len(inputs.shape) == 2: inputs = inputs.unsqueeze(1)
            optimizer.zero_grad()

            z = (0.05 + 1.90 * torch.rand(B, 1)).to(device); y = torch.randn(B, 1).to(device)
            x_di, cls_out, pred_domain = net(inputs, z, y)
            cls_ori, cls_new = torch.split(cls_out, B); di_ori, di_new = torch.split(x_di, B)
        ...
        net.eval(); all_labels, all_preds, all_probs = [], [], []
        with torch.no_grad():
    ...
    print(f"DACN_FINAL_ACCURACY:{np.mean(tail_acc):.2f}")
